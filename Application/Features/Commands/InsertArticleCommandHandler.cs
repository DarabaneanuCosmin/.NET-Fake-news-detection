    ...
    public class InsertArticleCommandHandler: DbConfiguration
    {
        ...
        public async void InsertArticle(InsertArticleCommand article)
        {
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand("Insert into article (id, id_user, title, text, subject, date)" +
                    " values(@id, @id_user, @title, @text, @subject, @date)", this.connection);
                cmd.Parameters.AddWithValue("@id", article.id);
                cmd.Parameters.AddWithValue("@id_user", article.id);
